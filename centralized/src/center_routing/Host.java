...
public class Host {
    ...
    private JSplitPane routingTopPanel;
    private JScrollPane linkStateField;              // Show online users
    private JPanel talkPanel;
    private JScrollPane mesPanel;                   // Show message 
    private JTextArea mesField;
    private JPanel pingPanel;                       // Write message to host
    ...
    private boolean active;                         // Whether connect to a controller
    private String CONTROL_IP;                       // IP to connect
    ...
    public Host() {
        ...
        linkStateField = new JScrollPane(linkState);
        linkStateField.setBorder(new TitledBorder("Links State (NeighborOrNot / Dest / Cost)"));
        sendNewLinksStateButton = new JButton("Update!");
        sendNewLinksStateButton.setEnabled(false);
        routingTopPanel = new JSplitPane(JSplitPane.VERTICAL_SPLIT, linkStateField, sendNewLinksStateButton);
        routingTopPanel.setDividerLocation(360);
        ...
        routingPanel = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, routingMessageField, routingTopPanel);
    ...
    private void sendNewLinksState() {
        ...
        for (int i = 0; i < dst_num; i++) {
            ...
            String cost;
            if (item.checkBox.isSelected()) {
                cost = item.field.getText();
                // System.out.println(dst + " " + cost + " " + cost.equalsIgnoreCase("inf"));
            ...
            while (active) {
                try {
                    ...
                    if ("ADD".equals(mode)) {
                        ...
                        updateLinkStateField(getContent(message));
                        linkState.updateUI();
    ...
    public void updateLinkStateField(String linkCost) {
